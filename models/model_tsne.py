...
def out(tensor):
    ...
    j = 0
    i = 0
    tens = torch.tensor([]).cuda()
    
    ten = torch.tensor([]).cuda()
...
def mixup_dis(preds, lamda, s_label=None):
    if s_label != None:
        ...
        label = torch.eye(preds.shape[0]).cuda()# dist.get_rank()
        mixup_loss = -torch.sum(label * F.log_softmax(preds, dim=1), dim=1)
        mixup_loss = torch.sum(torch.mul(mixup_loss, lamda))
    return mixup_loss
...
class VisionTransformer(nn.Module):
    ...
    def mix_source_target(self, s_token, t_token, bbx1, bby1, bbx2, bby2, s_label, s_scores, t_scores, s_logits, t_logits,
        ...
        t_scores = (torch.ones(20, 256) / 256).cuda()# dist.get_rank()
        s_lambda = mix_lambda_atten(s_scores, t_scores, a, 256)  # with attention map
        t_lambda = 1 - s_lambda

        s_onehot = torch.tensor(convert_to_onehot(s_label, 31), dtype=torch.float32).cuda()# dist.get_rank()
    ...
    def forward(self, x_s, x_t=None, ad_net=None, ad_net2=None, y_s=None):
        ...
        if x_t is not None:
            ...
            lam = dists.Beta(1, 1).rsample(
                (B, 1,)).cuda().squeeze(-1)# dist.get_rank()
            
            t = torch.full((B,1),256).cuda()
            ...
            cut_x = torch.randint(15,(20,1)).cuda()
            cut_y = torch.randint(15,(20,1)).cuda()
            ...
            s_lambda = torch.full((B,16,16),1).cuda()
            for i in range(B):
            ...
            s_onehot = torch.tensor(convert_to_onehot(y_s,31), dtype=torch.float32).cuda()# dist.get_rank()
