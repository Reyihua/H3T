...
def contrastive_loss(f_s, f_m, f_t, y_s, y_t, y_m=None, temperature=1.0):
    batchsize = f_s.size()[0]
    #print(batchsize)

    f_s = nn.functional.normalize(f_s, p=2, dim=1)
    f_m = nn.functional.normalize(f_t, p=2, dim=1)
    f_t = nn.functional.normalize(f_t, p=2, dim=1)
    
    mask = y_s == y_t

    zero = torch.zeros_like(y_s)
    result = torch.where(mask, y_s, zero)

    z = torch.cartesian_prod(y_s, y_t)
    r = []
# 使用scatter函数，根据条件将符合条件的元素设�?  
    ...
    labels = torch.Tensor(r).view(tensor).cuda(dist.get_rank())
    
    f_m =torch.transpose(f_m, 0, 1)
    logits_sm = torch.matmul(f_s, f_m)/temperature
    logits_tm = torch.matmul(f_t, f_m)/temperature
    
    #logits_sm = torch.log_softmax(logits_sm, dim=1)
    logits_sm = torch.nn.functional.softmax(logits_sm, dim=1)
    
    logits_sm = torch.mul(logits_sm, labels)
    
    logits_sm = torch.sum(logits_sm, dim=1)
    
    logits_sm = torch.log(logits_sm)
    
    loss_cont_sm = torch.sum(logits_sm)
    
    #logits_tm = torch.log_softmax(logits_tm, dim=1)
    logits_tm = torch.nn.functional.softmax(logits_tm, dim=1)
    
    logits_tm = torch.mul(logits_tm, labels)
    
    logits_tm = torch.sum(logits_tm, dim=1)
    
    logits_tm = torch.log(logits_tm)
    
    loss_cont_tm = torch.sum(logits_tm)
    
    return loss_cont_sm + loss_cont_tm
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=True, msa_layer=12):
        ...
        #self.lamda = nn.Parameter(torch.Tensor([0.475]))
        ...
        #self.super_ratio2 = nn.Parameter(torch.Tensor([-2]))
        #self.cross = CrossEntropy_softLabel(num_classes=31)
    
    def mix_source_target(self, s_token, t_token, bbx1, bby1, bbx2, bby2, s_label, s_scores, t_scores, s_logits, t_logits,
                          ad_net=None,ad_net2=None, posi=None, s_lambda=None):


        
        """source_token = s_token[:, 1:, :].view(20, 16, 16, 768)
        target_token = t_token[:, 1:, :].view(20, 16, 16, 768)
        mix_token = source_token
        
        ratio = (bbx2 - bbx1) * (bby2 - bby1)

        for i in range(20):
              mix_token[i, bbx1[i]:bbx2[i], bby1[i]:bby2[i], :] = target_token[i, bbx1[i]:bbx2[i], bby1[i]:bby2[i], :]
              s_token[i, 0, :] = s_token[i, 0, :].clone() *  (1 - ratio[i])
              t_token[i, 0, :] = t_token[i, 0, :].clone() * ratio[i]
        mix_t = s_token[:, 0, :].unsqueeze(dim=1) + t_token[:, 0, :].unsqueeze(dim=1)
        
        m_tokens = torch.cat((mix_t, mix_token.view(20, 256, 768)), dim=1)
        ...
        a = s_lambda
        ratio = ((bbx2 - bbx1) * (bby2 - bby1) //256).unsqueeze(dim=1)
        s_token1 = s_token
        t_token1 = s_token 
        """for i in range(20):
              s_token1[i, 0, :] = s_token[i, 0, :].clone() *  (1 - ratio[i])
              t_token1[i, 0, :] = t_token[i, 0, :].clone() * ratio[i]"""
        s_lambda = s_lambda.unsqueeze(dim=-1)
        source_token = torch.mul(s_token[:, 1:, :], s_lambda)
        src_token = torch.cat((s_token[:, 0, :].unsqueeze(dim=1) * ratio, source_token), dim=1)
        target_token = torch.mul(t_token[:, 1:, :], 1 - s_lambda)
        trg_token = torch.cat((t_token[:, 0, :].unsqueeze(dim=1) * ( 1 - ratio), target_token), dim=1)
        ...
        s_onehot = torch.tensor(convert_to_onehot(s_label, 345), dtype=torch.float32).cuda(dist.get_rank())# dist.get_rank()
    ...
    def forward(self, x_s, x_t=None, ad_net=None, ad_net2=None, y_s=None):
        ...
        if x_t is not None:
            ...
            lam = dists.Beta(1, 1).rsample(
                (B, 1,)).cuda(dist.get_rank()).squeeze(-1)# dist.get_rank()
            
            t = torch.full((B,1),256).cuda(dist.get_rank())
            patch_num = lam.unsqueeze(dim=1) * t
            
            
            
            patch_num1 = torch.sqrt(patch_num)
            patch_num2 = torch.as_tensor(patch_num1, dtype=torch.int64)
            cut_x = torch.randint(15,(20,1)).cuda(dist.get_rank())
            cut_y = torch.randint(15,(20,1)).cuda(dist.get_rank())
            
            bbx1 = torch.clamp( cut_x - patch_num2//2, 0, 15)
            bby1 = torch.clamp( cut_y - patch_num2//2, 0, 15)
            bbx2 = torch.clamp( cut_x + patch_num2//2, 0, 15)
            bby2 = torch.clamp( cut_y + patch_num2//2, 0, 15)
            ...
            s_lambda = torch.full((B,16,16),1).cuda(dist.get_rank())
            for i in range(B):
                   s_lambda[i, bbx1[i]:bbx2[i], bby1[i]:bby2[i]] = 0
            s_lambda = s_lambda.view(B,256)
            mixup_loss, m_s_t_logits, loss_ad_m = self.mix_source_target(s_embedding_output, t_embedding_output, bbx1, bby1, bbx2, bby2, y_s, s_scores,
                                                t_scores, x_s[:, 0], x_t[:, 0], ad_net, ad_net2, s_posi_emb, s_lambda)
            
            s_onehot = torch.tensor(convert_to_onehot(y_s,345), dtype=torch.float32).cuda(dist.get_rank())# dist.get_rank()
            s_lambda = mix_lambda_atten(s_scores, t_scores, s_lambda, 256)
            m1 = s_lambda.unsqueeze(dim=-1)*s_onehot
            t = 1-s_lambda.unsqueeze(dim=-1)
            
            y_t = torch.argmax(self.head(x_t[:,0]), dim=-1)
            #print(y_t)
            """if (torch.sum(y_t)==0):
                loss = 0
            else: 
                loss = contrastive_loss(x_s[:,0], m_s_t_logits[:,0], x_t[:,0], y_s, y_t, temperature=1.0)"""
            ...
            return logits_s, logits_t, (loss_ad_s + loss_ad_t) / 2.0 , loss_rec, x_s, x_t, mixup_loss, loss_m
